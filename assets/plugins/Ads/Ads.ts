//Usaremos el Patron Singleton, porque cada instancia de ads debe de ser unico (para no repetir un mismo anuncio)
...
class Ads {
    ...
    static getInstance(){//Aqui se hace evidente el patron singleton, solamente por aqui podemos obtener la instancia la cual se asegura por medio del metod que solo exista una instancia, sino retorna la misma instancia ya creada
        if(!Ads.instance) {
    ...
    private initAds() {
        this.ads = [...ALL_ADS]//Hacemos un spreadOperator para hacer una copia del arreglo y mantener la original intacta
    }

    getAd() {
        if(this.ads.length === 0){ //Si se nos acaban nuestros anuncios, lo volvemos a inicializar
            this.initAds();
        }
        return this.ads.pop()
    }
